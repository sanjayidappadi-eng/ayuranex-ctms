...
type RecipientTargetKey = "handset" | "dcgi" | "iec" | "npvcc" | "pi";
...
export default function AdminSecurityAudit({ userRole }: AdminSecurityAuditProps) {
  ...
  const [testTargetKey, setTestTargetKey] = useState<RecipientTargetKey>("handset");
  ...
  const loadNotificationSettings = async () => {
    try {
      ...
      // Strictly enforce Fast2SMS and populate handset defaults
      cfg.smsProvider = "fast2sms";
      cfg.userMobile = cfg.userMobile || "+91-7094258926";
      cfg.userEmail = cfg.userEmail || cfg.smtpUser || "sanjayidappadi@gmail.com";
      setNotifConfig(cfg);
    } catch (err) {
  ...
  const TARGET_DETAILS: Record<RecipientTargetKey, { title: string; email: string; mobile: string; authority: string }> = {
    handset: {
      title: "Personal Handset & Safety Officer Desk (+91-7094258926)",
      email: notifConfig.smtpUser || notifConfig.userEmail || "sanjayidappadi@gmail.com",
      mobile: notifConfig.userMobile || "+91-7094258926",
      authority: "Direct Physical SIM Card (+91-7094258926) & Personal Gmail App"
    },
    dcgi: {
...
This is an official statutory electronic notification generated by AyuraNex CTMS & National Pharmacovigilance Coordination Centre (NPvCC).`;
  ...
  const handleExecuteAutoDispatch = async () => {
    ...
    const activeSmtpUser = (notifConfig.smtpUser || "sanjayidappadi@gmail.com").trim();
    ...
    try {
      ...
      if (emailResult.success) {
        if (hasSmtpPass) {
          addLog(`>> [REAL EMAIL DELIVERED TO INBOX] Sent via Gmail SMTP to ${target.email}! Message-ID: ${emailResult.messageId}. Check your phone's Gmail app now!`);
        } else {
        ...
        addLog(`>> NOTE: Google requires a 16-character App Password from https://myaccount.google.com/apppasswords. Ensure 2-Step Verification is active on ${activeSmtpUser}.`);
      }
                  ...
                  <button
                    ...
                    onClick={() => {
                      const updated = {
                        ...
                        smtpPort: "465",
                        smtpUser: (notifConfig.smtpUser || "sanjayidappadi@gmail.com").trim(),
                      ...
                      try {
                        ...
                        alert("🔒 Credentials saved and secured! Dispatches will now route directly through Fast2SMS and Gmail.");
                      } catch (err) {
                        console.error(err);
                      }
                    }}
              ...
              >
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-[11px] opacity-90 leading-relaxed">
                    {notifConfig.fast2SmsApiKey && notifConfig.fast2SmsApiKey.trim().length > 5
                      ? "Fast2SMS API key connected. Transmits cellular SMS packets over Airtel/Jio/Vi/BSNL towers directly to +91-7094258926."
                      : "Enter Fast2SMS Authorization Key below to deliver live SMS alerts to your physical phone SIM (+91-7094258926)."}
                  </p>
          ...
          <div className="w-full max-w-3xl rounded-3xl border border-teal-500/50 bg-slate-900 p-6 sm:p-7 shadow-2xl space-y-5 animate-in fade-in zoom-in-95 duration-200 max-h-[92vh] overflow-y-auto">
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-wider text-slate-400">
                Choose Recipient Authority to Target:
              </label>
              <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
                {(["handset", "dcgi", "iec", "npvcc", "pi"] as RecipientTargetKey[]).map((key) => {
                  const target = TARGET_DETAILS[key];
                  const isSelected = testTargetKey === key;
                  const isHandset = key === "handset";
                  return (
                    <button
                      key={key}
                      type="button"
                      ...
                      className={`p-2.5 rounded-xl text-left border text-xs font-semibold transition cursor-pointer ${
                        isSelected
                          ? isHandset
                            ? "bg-emerald-500/20 text-emerald-300 border-emerald-500/80 shadow-md ring-1 ring-emerald-400/50"
                    ...
                    >
                      <p className={`font-bold truncate flex items-center gap-1 ${isHandset ? "text-emerald-300" : "text-white"}`}>
                        {isHandset && <Phone size={11} className="text-emerald-400 shrink-0" />}
                        {isHandset ? "HANDSET (ME)" : key.toUpperCase()}
                      </p>
                      <p className="text-[10px] text-slate-400 truncate mt-0.5">{isHandset ? target.mobile : target.email}</p>
                    </button>
                  );
                })}
              </div>
                      ...
                      <div className="space-y-1">
                        ...
                        {isCredentialsLocked ? (
                          ...
                          <p className="text-[10px] text-emerald-400 font-medium">
                            ✓ Official clinical dossier delivered to sanjayidappadi@gmail.com.
                          </p>
                        )}
                      </div>
                    </div>
